...
class PreferencesService {
  ...
  /**
   * Salvar preferências (criar ou atualizar)
   */
  async savePreferences(clientId, preferences) {
    // Verificar se já existe
    const existing = await this.getPreferences(clientId);

    if (existing) {
      return this.updatePreferences(clientId, preferences);
    } else {
      return this.createPreferences(clientId, preferences);
    }
  }
   ...
   */
  async createPreferences(clientId, preferences) {
    return new Promise((resolve, reject) => {
      this.db.run(
  ...
  async updatePreferences(clientId, preferences) {
    return new Promise((resolve, reject) => {
      this.db.run(
        ...
        function(err) {
          if (err) reject(err);
          else resolve({ updated: true, changes: this.changes });
        }
      );
    });
  }
   ...
   */
  async deletePreferences(clientId) {
    return new Promise((resolve, reject) => {
      this.db.run(
